...
public class TestCustomModelMapping extends AbstractTest {
    ...
    protected void doInitialization() {
        ...
        config.customModelMapping(User.class, new Function<EmberModel, EmberModel>() {
            @Override
            public EmberModel apply(EmberModel original) {
                if (original.getSuperClass().equals("DS.Model")) {
                    return new EmberModel(original.getName(), "App.AbstractUser",
                            original.getAttributes(), original.getWriteFileHeader());
                } else {
                    return original;
                }
            }
